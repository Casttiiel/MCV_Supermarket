...
)
{
  ...
  output.Pos.y += 0.1 * sin( GlobalWorldTime );
...
float4 PS(VS_OUTPUT input) : SV_Target
{
  ...
  amount_of_water_color = 0.05;
  float4 water_color = ObjColor;
  float4 base_color = noise0 * water_color * amount_of_water_color + ( 1 - amount_of_water_color) * albedo_color * ObjColor;

  //amount_of_water_color * water_color * noise0  
