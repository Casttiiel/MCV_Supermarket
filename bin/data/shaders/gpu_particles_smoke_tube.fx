...
TInstance spawnParticle( uint unique_id ) {
  ...
  float  rnd4 = rand(unique_id + 75) * 360.0f;
...
[numthreads(1, 1, 1)]
void cs_particles_smoke_tube_spawn( 
...
void cs_particles_smoke_tube_update( 
  uint thread_id : SV_DispatchThreadID,
...
)
{
  ...
  float deg = instance.dummy2 * PI / 180.0f;
  float2x2 m = {cos(deg),-sin(deg),sin(deg),cos(deg)};
  float4 rotatedPos = float4(mul(m,input.Pos.xz),input.Pos.yw);
  float3 p = instance.pos + rotatedPos * instance.scale; //multiply localPos to scale the billboard
  /* 
...
float4 PS(v2p input) : SV_Target {
  ...
  float alpha_erosion_entrance = input.time / 0.3f;
