...
float4 PS(v2p input) : SV_Target {
  const float2x2 rot_matrix = { cos(GlobalWorldTime * input.dir), -sin(GlobalWorldTime * input.dir),
    sin(GlobalWorldTime * input.dir), cos(GlobalWorldTime * input.dir)
  };
  float2 rot_uv = mul(input.Uv - float2(0.5f,0.5f), rot_matrix);
  float4 texture_color = txAlbedo.Sample(samLinear, input.Uv);
  float4 snowflake = txNormal.Sample(samLinear, rot_uv + float2(0.5f,0.5f));
  //return input.Color * 4;
  return texture_color * input.Color * snowflake; // + float4( 1,1,1,0);
}
