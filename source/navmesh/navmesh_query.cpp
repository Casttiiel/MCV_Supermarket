...
std::vector<VEC3> CNavmeshQuery::findPath(VEC3 start, VEC3 end) {
	...
	//actualizar busqueda de poligonos cercanos
		...
		while (npolys && m_nsmoothPath < MAX_SMOOTH) {
			...
			if (endOfPath && inRange(&iterPos.x, steerPos, SLOP, 1.0f)) {
				...
				if (dtStatusSucceed(status)) {
					...
					//dtVcopy(&iterPos.x, &endPos.x);
					iterPos = endPos;
					float eh = 0.0f;
					data->m_navQuery->getPolyHeight(polys[0], &iterPos.x, &eh);
					iterPos.y = eh;//en pos Y de vector 
				}
			}
...
float CNavmeshQuery::wallDistance(VEC3 pos) {
	...
	if (m_startRef) {
		...
		float m_distanceToWall = -99999.f;
...
bool CNavmeshQuery::raycast(VEC3 start, VEC3 end,VEC3 &m_hitPos) {
	//m_nstraightPath = 0;
...
*/
	...
	if (t > 1) {
		// No hit
		//dtVcopy(&m_hitPos.x, &end.x);
		m_hitPos = end;
		m_hitResult = false;
	}
	else {
		// Hit
		dtVlerp(&m_hitPos.x, &start.x, &end.x, t);
		m_hitResult = true;
	}
	// Adjust height.
	if (m_npolys > 0) {
		float h = 0;
		data->m_navQuery->getPolyHeight(m_polys[m_npolys - 1], &m_hitPos.x, &h);
		m_hitPos.y = h;
	}
