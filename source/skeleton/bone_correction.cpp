...
// -----------------------------------------
QUAT getRotationFromAToB(VEC3 A, VEC3 B, float unit_amount,float maxAngle) {
  ...
  angle = clamp(angle, 0.f, deg2rad(maxAngle * unit_amount));
...
void TBoneCorrection::apply(CalSkeleton* skel, VEC3 dx_world_target, float external_unit_amount, float angleMax) const {
  ...
  CalQuaternion q_rot = DX2Cal(getRotationFromAToB(local_axis_to_correct, Cal2DX(local_dir), amount * external_unit_amount,angleMax));
